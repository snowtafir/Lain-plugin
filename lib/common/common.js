...
function array (data) {
  ...
  if (data?.[0]?.data?.type === 'test' || data?.[1]?.data?.type === 'test') {
    ...
    msg = [].concat(...data.map(i => ((typeof i === 'string' || typeof i === 'number')
      ? [{ type: 'text', text: String(i) }]
      : Array.isArray(i)
        ? [].concat(...i.map(format => ((typeof i === 'string' || typeof i === 'number')
...
function getUrls (url, exclude = []) {
  ...
  try {
    ...
    lain.info('Lain-plugin', '没有安装 get-urls 模块，建议执行pnpm install -P 进行安装使用更精准的替换url')
