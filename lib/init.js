...
const isNull = i => i !== null && i !== '' && i !== undefined
...
global.lain = {
  ...
  */
  ...
  info: function (id, ...log) {
    logger.info(...[this.nickname(id) || '', ...log].filter(isNull))
  },
  mark: function (id, ...log) {
    logger.mark(...[this.nickname(id) || '', ...log].filter(isNull))
  },
  error: function (id, ...log) {
    logger.error(...[this.nickname(id) || '', ...log].filter(isNull))
  },
  warn: function (id, ...log) {
    logger.warn(...[this.nickname(id) || '', ...log].filter(isNull))
  },
  debug: function (id, ...log) {
    logger.debug(...[this.nickname(id) || '', ...log].filter(isNull))
  },
  trace: function (id, ...log) {
    logger.trace(...[this.nickname(id) || '', ...log].filter(isNull))
  },
  fatal: function (id, ...log) {
    logger.fatal(...[this.nickname(id) || '', ...log].filter(isNull))
  },
  em: function (name = '', data) {
    while (true) {
      Bot.emit(name, data)
