...
class Cfg {
  ...
  getToken (type, appid = 'all') {
    ...
    if (config?.[type]?.[appid]) {
      return { ...(defSet?.[type.replace("_Token", "_Default")]?.default || {}), ...(config?.[type.replace("_Token", "_Default")]?.default || {}), ...config[type][appid] }
    }
    return { ...(defSet?.[type.replace("_Token", "_Default")] || {}), ...(config?.[type.replace("_Token", "_Default")] || {}), ...(config?.[type] || {}) }
  }
