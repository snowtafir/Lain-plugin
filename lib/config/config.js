...
class Cfg {
  ...
  async lodCfg () {
    ...
    if (fs.existsSync(QQBot)) {
      state = true
      const config = new YamlHandler(this.path + 'Config-Token.yaml')
  ...
  /** QQ群、频道机器人token配置 */
  getToken (appid = 'all') {
    let config = this.getConfig('Config-Token')
    if (config.QQ_Token?.[appid]) {
      return config.QQ_Token[appid]
    }
    return config.QQ_Token || {}
  }
