...
async function imgProc (file) {
  ...
  if (!url) url = `http://${QQBotImgIP}:${QQBotPort || port}/api/QQBot?token=${QQBotImgToken}&name=${time}`
...
async function uploadFile (file) {
  if (!(Buffer.isBuffer(file) || file instanceof Uint8Array)) {
    if (file.startsWith('file://') || !file.startsWith('base64://')) {
...
async function uploadQQ (file, uin = Bot.uin) {
  ...
  try {
    ...
    await Bot[uin].pickUser(uin).recallMsg(ret.message_id)
  } catch (err) { throw err; }
