...
export default async function createAndStartBot(cfg) {
  try {
    ...
    bot.on("message.group", async (e) => {
      Bot[cfg.appid].stat.recv_msg_cnt++
      return Bot.em("message.group", await message.msg(e, true))
    })

    // 私聊被动回复
    bot.on("message.private", async (e) => {
      Bot[cfg.appid].stat.recv_msg_cnt++
      return Bot.em("message.private", await message.msg(e, false))
    })
...
async function LoadBot(appID, bot) {
  ...
  try {
    ...
    glList.forEach(async i => {
      ...
      Bot[appID].gl.set(group_id, { group_id })
    })
  } catch { }
}
