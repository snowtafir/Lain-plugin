    ...
    for (let i of msg) {
      ...
      switch (i.type) {
        case "text":
          ...
          break
        case "image":
          logger.info(`${logger.blue(`[${data.self_id}]`)} 发送图片：[${id}] ${file.name}(${file.url} ${(file.buffer.length/1024).toFixed(2)}KB)`)
          break
        case "record":
          logger.info(`${logger.blue(`[${data.self_id}]`)} 发送音频：[${id}] ${file.name}(${file.url} ${(file.buffer.length/1024).toFixed(2)}KB)`)
          break
        case "video":
          logger.info(`${logger.blue(`[${data.self_id}]`)} 发送视频：[${id}] ${file.name}(${file.url}) ${(file.buffer.length/1024).toFixed(2)}KB`)
          break
        case "reply":
          break
        case "at":
