...
export default class guild {
    ...
    async monitor() {
        ...
        /** 告知用户已连接成功 */
        await common.info(this.id, "连接成功，正在加载资源中...")
        /** 延迟下 */
        await common.sleep(200)
        /** 获取一些基本信息 */
        await this.guilds(this.id)
        /** 告知用户加载资源完成 */
        await common.info(this.id, "加载资源完毕...")
    }
    ...
    async guilds() {
        ...
        for (let qg of meGuilds) {
            ...
            try {
                ...
                await common.warn(this.id, `Bot无法在频道 ${qg.id} 中读取基础信息，请给予权限...错误信息：${err.message}`, "error")
            }
            ...
            try {
                ...
                await common.warn(this.id, `Bot无法在频道 ${qg.id} 中读取子频道列表，请给予权限...错误信息：${err.message}`, "error")
            }
    ...
    Sendprivate = async (data) => {
        ...
        await common.info(this.id, `发送私信消息：${hi}`)
