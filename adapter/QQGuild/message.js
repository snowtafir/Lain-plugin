...
export default class message {
  ...
  async msg(type = "") {
    ...
    const member = {
      ...
      /** 踢 */
      kick: async () => await Bot[id].client.guildApi.deleteGuildMember(msg.guild_id, msg.author.id)
    }
    ...
    if (type === "私信") {
      e.friend = {
        sendMsg: async (msg, quote) => await this.reply(msg, quote, group_name),
        recallMsg: async (msg_id) => await this.recallMsg(msg.channel_id, msg_id),
        makeForwardMsg: async (forwardMsg) => await common.makeForwardMsg(forwardMsg, false, this.data),
        getChatHistory: async (msg_id, num) => {
          const source = await this.getChatHistory(msg.channel_id, msg_id)
          return [source]
        },
        getAvatarUrl: async () => msg.author.avatar
      }
    } else {
      e.group = {
        is_admin,
    ...
    /** 快速撤回 */
    e.recall = async () => await this.recallMsg(msg.channel_id, msg.id)
    /** 快速回复 */
    e.reply = async (msg, quote) => await this.reply(msg, quote, group_name)
    /** 将收到的消息转为字符串 */
    e.toString = () => toString
